...
//lpg 2020-12-29
/**
 * 从网页上下载当前window中的所有表格到文件中
 * @param {name} name 要保存的文件的名字要带有后缀.xlsx
 */
function downloadTables (name){
	let tables =Array.from($('table'));
	let sheets=tables.map(table=>XLSX.utils.table_to_sheet(table));
	let book=XLSX.utils.book_new();
	sheets.forEach(sheet=>XLSX.utils.book_append_sheet(book,sheet));
	XLSX.writeFile(book,name);
}

XLSX.downloadTables=downloadTables;
